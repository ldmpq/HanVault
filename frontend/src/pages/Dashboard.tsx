...
import { Play, Flame, RotateCw, Clock, PlusSquare, ChevronLeft, ChevronRight, List, Lock, Loader2, GraduationCap } from 'lucide-react';
...
export default function Dashboard() {
  ...
  if (error || !data) return <div className="bg-red-50 text-red-600 p-4 rounded-xl text-center font-medium">{error || 'Không có dữ liệu.'}</div>;
  ...
  return (
    <div className="w-full max-w-[1200px] mx-auto animate-fade-in pb-12">
      ...
      <div className="bg-gradient-to-br from-[#FFFBFB] to-red-50/30 rounded-[2rem] p-10 flex flex-col md:flex-row justify-between items-center shadow-[0_4px_20px_rgb(0,0,0,0.02)] border border-gray-100 mb-8 gap-8">
        <div className="max-w-xl">
          <h1 className="text-4xl font-bold text-gray-900 mb-4 tracking-tight">
            {greeting}, <span className="text-[#A82B2B]">{data.userName || 'Lão Tử'}</span>
          </h1>
      ...
      {/* MIDDLE SECTION (Khôi phục toàn bộ) */}
        ...
        <div className="lg:col-span-8 bg-white rounded-[2rem] p-8 shadow-sm border border-gray-100 flex flex-col justify-between">
          ...
          <div className="flex flex-col md:flex-row gap-8 items-center h-full">
            <div className="w-full md:w-1/3 h-full min-h-[220px] bg-gray-100 rounded-2xl overflow-hidden relative shadow-sm">
              <img src="https://images.unsplash.com/photo-1541959833400-049d37f98ccd?w=400&q=80" alt="HSK 4 Book" className="w-full h-full object-cover opacity-90 mix-blend-multiply" />
      ...
      {/* RECOMMENDED DECKS (Khôi phục toàn bộ) */}
      <div className="mb-8">
      ...
      {/* WEEKLY ACTIVITY HEATMAP (Khôi phục toàn bộ) */}
        ...
        <div className="flex gap-3 items-start overflow-x-auto custom-scrollbar pb-2">
          ...
          <div className="grid grid-rows-7 gap-y-1.5 gap-x-1 grid-flow-col flex-1">
            {activityData.map((level, i) => {
              ...
              return <div key={i} className={`w-4 h-4 rounded-sm ${bgColors[level]} hover:ring-2 hover:ring-gray-300 transition-all cursor-pointer`}></div>;
            })}
          </div>
        </div>
      </div>
