...
export default function SearchFilterBar({
  ...
  return (
    <>
      ...
      {/* Hiển thị Badges Lọc trực tiếp dưới thanh Search */}
      {(hskFilter || topicFilter || showFavorites) && (
        <div className="flex flex-wrap items-center justify-center gap-2 mt-2 mb-6">
          ...
          {hskFilter && <span className="bg-red-50 text-[#A82B2B] px-3 py-1 rounded-full text-xs font-medium border border-red-100">{hskOptions.find(o => o.value === hskFilter)?.label}</span>}
          {topicFilter && <span className="bg-red-50 text-[#A82B2B] px-3 py-1 rounded-full text-xs font-medium border border-red-100">{topics.find(t => t.id.toString() === topicFilter)?.name}</span>}
          {showFavorites && <span className="bg-red-50 text-[#A82B2B] px-3 py-1 rounded-full text-xs font-medium border border-red-100 flex items-center gap-1"><Heart className="w-3 h-3 fill-current" /> Yêu thích</span>}
          <button 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-2xl bg-[#F4F4F5] rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-fade-in max-h-[90vh]">
            <div className="flex items-center justify-between p-5 border-b border-gray-200/60 bg-white">
              ...
              <button onClick={() => setIsModalOpen(false)} className="text-gray-400 hover:text-gray-700 bg-gray-100 hover:bg-gray-200 p-1.5 rounded-md transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar flex-1 text-left">
                ...
                <div className="mb-8">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {recentSearches.map((term, idx) => (
                      <button
                        key={idx}
                        onClick={() => { onSearchChange(term); setIsModalOpen(false); }}
                        className="bg-white border border-gray-200 text-gray-600 px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-50 flex items-center gap-2 transition-colors shadow-sm"
                      >
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                  <button
                    onClick={() => onHskFilterChange('')}
                    className={`py-2.5 rounded-xl text-sm font-medium transition-colors border ${hskFilter === '' ? 'bg-[#A82B2B] text-white border-[#A82B2B] shadow-md' : 'bg-white text-gray-600 border-transparent hover:border-gray-200'}`}
                  >
                    Tất cả
                  </button>
                  {hskOptions.map(option => (
                    <button
                      key={option.value}
                      onClick={() => onHskFilterChange(option.value)}
                      className={`py-2.5 rounded-xl text-sm font-medium transition-colors border flex items-center justify-center gap-1.5 ${hskFilter === option.value ? 'bg-[#A82B2B] text-white border-[#A82B2B] shadow-md' : 'bg-white text-gray-600 border-transparent hover:border-gray-200 shadow-sm'}`}
                    >
                      {hskFilter === option.value && <CheckCircle2 className="w-4 h-4" />}
                      {option.label}
                    </button>
                  ))}
                </div>
              ...
              <div className="mb-8">
                ...
                <div className="bg-white p-1 rounded-xl shadow-sm border border-gray-100">
                  <select 
                    ...
                    className="w-full bg-transparent border-none text-gray-700 py-3 px-4 rounded-lg text-sm font-medium focus:outline-none focus:ring-2 focus:ring-red-100 cursor-pointer"
                  >
                    <option value="">Tất cả chủ đề</option>
                    {topics.map(t => (
                      <option key={t.id} value={t.id}>{t.name} ({t.vocabCount} từ)</option>
                    ))}
                  </select>
                </div>
              </div>
              ...
              <div>
                ...
                <button 
                  ...
                  className={`w-full sm:w-auto py-3 px-6 rounded-xl text-sm font-medium flex items-center justify-center gap-2 transition-all duration-200 border ${
                    showFavorites ? 'bg-red-50 border-red-200 text-[#A82B2B] shadow-sm' : 'bg-white border-transparent text-gray-600 hover:border-gray-200 shadow-sm'
                  }`}
                >
                  <Heart className="w-5 h-5" fill={showFavorites ? "#A82B2B" : "none"} color={showFavorites ? "#A82B2B" : "currentColor"} /> 
                  Từ vựng Yêu thích
                </button>
              </div>
            ...
            <div className="p-5 border-t border-gray-200/60 bg-white flex justify-end gap-3">
              ...
              <button 
                ...
                className="px-8 py-2.5 rounded-xl text-sm font-bold bg-[#A82B2B] text-white hover:bg-[#8b2323] shadow-md transition-colors"
              >
                Ok
              </button>
            </div>
